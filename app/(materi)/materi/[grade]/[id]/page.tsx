...
async function getMateri(id: number, grade: number) {
  const contentCookie = await cookies();
  const supabase = createClient(contentCookie);
  const { data: raw } = await supabase.from("materials").select("content, slug").eq("id", id);
  if (!raw) {
    return;
  }
  const content = String(raw[0].content);
  const toc: TocItem[] = [];
    ...
    .use({
      renderer: {
        ...
        image(token: Tokens.Image) {
          const src = resolveImageSrc(token.href, grade);
          return markdownImageFigure(src, token.text, token.title);
        },
      },
    })
    .parse(content) as string;
  const title = toc[0] ?? raw[0].slug.replace(/-/g, " ");
...
export default async function MateriDetail({ params }: PageProps) {
  ...
  const materi = await getMateri(id, grade);
